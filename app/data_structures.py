...
from typing import Any, List, Optional, TypedDict # Added List, Optional, TypedDict
...
class AgentState(TypedDict, total=False):
    ...
    This TypedDict holds all the information that is passed between nodes in the graph.
    Using `total=False` means keys are not required to be present, suitable for
    a state that evolves as the graph progresses.
    """
    task_id: Required[str]
    """The unique identifier for the current task."""

    repo_url: Optional[str]
    """URL of the Git repository to be cloned for the task."""

    local_repo_path: Optional[str]
    """Local file system path to the cloned repository."""

    current_branch: Optional[str]
    """The name of the current Git branch being worked on."""

    commit_hash: Optional[str]
    """The specific commit hash the repository should be checked out to."""

    analysis_results: Optional[Any]
    """Output from the repository analysis step (e.g., structure, dependencies)."""

    fix_plan: Optional[Any] # Could be a string (Markdown plan) or a structured object
    """The plan generated by the LLM for fixing the issue or implementing the feature."""

    applied_fixes: Optional[List[dict]]
    """A list of dictionaries, each detailing an applied fix (e.g., file path, summary of change)."""

    generated_tests: Optional[List[dict]]
    """A list of dictionaries, each detailing a generated test file (e.g., path, summary)."""

    test_results: Optional[Any] # Could be a string (test runner output) or structured test report
    """The results obtained from running tests after applying fixes or generating new tests."""

    generated_docs: Optional[List[dict]]
    """A list of dictionaries, each detailing generated or updated documentation (e.g., file path)."""

    pr_url: Optional[str]
    """URL of the pull request created, if any."""

    deployment_status: Optional[str]
    """Status of any deployment attempt (e.g., "success", "failure", "pending")."""

    final_summary_message: Optional[str]
    """A final summary message about the outcome of the agent's run."""

    error_message: Optional[str]
    """Stores any error message if a step in the graph fails."""

    log_messages: List[str]
    """An accumulation of key log messages or events during the agent's run for summary purposes."""

    selected_model: Required[str]
    """The name of the LLM model selected for the current task run."""

    github_token_available: Required[bool]
    """Flag indicating if a GitHub token is available for operations requiring authentication."""

    # Example of how to make a field required with TypedDict if needed for some core fields
    # from typing_extensions import Required
    # initial_prompt: Required[str]
    # However, for LangGraph state, most things are optional as they are filled progressively.
    # For required fields from the start, they should be passed in during graph invocation.
    # For this example, making task_id, selected_model, and github_token_available as notionally required at start.
    # If using Python 3.9+, can use typing.Required. For older, typing_extensions.Required or just make them Optional.
    # For simplicity without adding typing_extensions, we'll assume they are passed in correctly.
    # The `total=False` makes all keys optional by default unless Required is used (Python 3.11+ for Required directly in typing).
    # If strictness is needed for some initial fields on older Python, a Pydantic model might be better.
    # For now, this TypedDict with `total=False` is flexible for LangGraph.
    # Let's assume for now that task_id, selected_model, github_token_available are populated at graph start.
    # If a field should *always* be present after a certain point, the node providing it is responsible.
    # Using Required from typing if Python 3.11+ is assumed, else they are effectively Optional.
    # For broad compatibility, making them Optional and ensuring they are set is safer if not using typing_extensions.
    # Let's make them Optional for now to avoid import issues with `Required` on older Pythons.
    # The user of the state graph will need to ensure initial population of critical fields.

    # Re-declaring with Optional for broader compatibility without typing_extensions.Required
    # And initializing log_messages to an empty list would typically be done by the graph's first node or input.
    # TypedDict itself doesn't handle default values.
    task_id: str # This should be provided at the start.
    selected_model: str # This should be provided at the start.
    github_token_available: bool # This should be provided at the start.
    log_messages: List[str] # Should be initialized to [] by the caller or first node.
